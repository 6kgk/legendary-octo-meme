...
export class PaymentService {
   ...
   * 发起支付 (模拟连接支付网关)
   */
  async createPaymentOrder(userId: string, plan: 'premium' | 'vip'): Promise<PaymentTransaction> {
   ...
   * 确认支付 (模拟支付成功回调)
   */
  async confirmPayment(transactionId: string): Promise<{ success: boolean; userProfile: any }> {
    const transaction = this.transactions.get(transactionId);
    if (!transaction) throw new Error('订单不存在');
    ...
    const updatedUser = await this.userService.upgradeSubscription(transaction.userId, level as any);
